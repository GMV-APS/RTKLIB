...
const prcopt_t prcopt_default={ /* defaults processing options */
    ...
    2,SYS_GPS|SYS_GLO|SYS_GAL,0,/* nf, navsys, restartp */
    15.0*D2R,{{0,0}},           /* elmin,snrmask */
    0,3,0,3,1,0,1,              /* sateph,modear,combar,glomodear,gpsmodear,bdsmodear,arfilter */
    20,0,4,5,10,20,             /* maxout,minlock,minfixsats,minholdsats,mindropsats,minfix */
    1,0.5,1.0,                  /* armaxiter,forgetion,afgainion */
    1,1,2,0,                    /* estion,esttrop,dynamics,tidecorr */
    1,0,0,0,0,                  /* niter,codesmooth,intpref,sbascorr,sbassatsel */
    0,0,                        /* rovpos,refpos */
    {300.0,300.0,300.0},        /* eratio[] */ /* WARNING: Needs to be updated if NFREQ changed*/
    300.0,                    /* dratio */
    {100.0,0.003,0.003,0.0,1.0,52.0,0.0,0.0}, /* err[-,base,el,bl,dop,snr_max,snr,rcverr] */
    {30.0,0.03,0.01},            /* std[] */
    {1E-4,1E-3,1E-4,1E-1,0.0,1000.0,0.2,0.005}, /* prn[] */
    5E-12,                      /* sclkstab */
    {3.0,0.25,0.0,1E-9,1E-5,3.0,3.0,0.0}, /* thresar */
    0.0,0.0,0.05,0,             /* elmaskar,elmaskhold,thresslip,thresdop, */
    0.1,0.01,30.0,120.0,        /* varholdamb,gainholdamb,maxtdif,maxoutage */
    {5.0,30.0,20.0},            /* maxinno {phase,code,doppler} */
    1,0,                          /* adaptive rescheck enable, rescheck min iter */
...
/*	 L1/E1/B1I    L5/E5a/B2a L2/E5b/B3I   E6/LEX/B2b E5(a+b)/B2ab         */
	{"PYWMNCLSX","QXI"     	,"PWYMNDCLSX",""       	,""       ,""}, /* GPS */
	{"CPABX"    ,"QXI"     	,"CPABX"     ,""       	,""       ,""}, /* GLO */
	{"CBXZA"    ,"QXI"     	,"QXI"       ,"CBXZA"  	,"QXI"    ,""}, /* GAL */
	{"CLSXZ"    ,"QXIDPZ"  	,"LSX"       ,"LSXEZ"  	,""       ,""}, /* QZS */
	{"C"        ,""        	,"QXI"       ,""       	,""       ,""}, /* SBS */
	{"IQXPDAN"  ,"PDX"     	,"IQXDPZ"    ,"DPZIQX" 	,"DPX"    ,""}, /* BDS */
	{"ABCX"     ,""        	,"ABCX"      ,""       	,""       ,""}  /* IRN */
};
#else
static char codepris[7][MAXFREQ][16]={  /* code priority for each freq-index */
/*   L1/E1/B1I    L2/E5b/B3I  L5/E5a/B2a  E6/LEX/B2b E5(a+b)/B2ab         */
    {"PYWMNCLSX","PWYMNDCLSX","QXI"      ,""        ,""       ,""}, /* GPS */
    {"CPABX"    ,"CPABX"     ,"QXI"      ,""        ,""       ,""}, /* GLO */
    {"CBXZA"    ,"QXI"       ,"QXI"      ,"CBXZA"   ,"QXI"    ,""}, /* GAL */
    {"CLSXZ"    ,"LSX"       ,"QXIDPZ"   ,"LSXEZ"   ,""       ,""}, /* QZS */
    {"C"        ,"QXI"       ,""         ,""        ,""       ,""}, /* SBS */
    {"IQXPDAN"  ,"IQXDPZ"    ,"PDX"      ,"DPZIQX"  ,"DPX"    ,""}, /* BDS */
    {"ABCX"     ,"ABCX"      ,""         ,""        ,""       ,""}  /* IRN */
};
#endif
...
static int code2freq_GPS(uint8_t code, double *freq)
{
    ...
    switch (obs[0]) {
        ...
        case '2': *freq=FREQL2; return FREQ_L1L5?2:1; /* L2 */
        case '5': *freq=FREQL5; return FREQ_L1L5?1:2; /* L5 */
    }
    return -1;
}
...
static int code2freq_GAL(uint8_t code, double *freq)
{
    ...
    switch (obs[0]) {
        ...
        case '7': *freq=FREQE5b; return FREQ_L1L5?2:1; /* E5b */
        case '5': *freq=FREQL5; return FREQ_L1L5?1:2; /* E5a */
...
static int code2freq_BDS(uint8_t code, double *freq)
{
    ...
    switch (obs[0]) {
        ...
        case '6': *freq=FREQ3_CMP; return FREQ_L1L5?2:1; /* B3 */
        case '5': *freq=FREQL5;     return FREQ_L1L5?1:2; /* B2a */
...
extern int getcodepri(int sys, uint8_t code, const char *opt)
{
    ...
    for (p=opt;p&&(p=strchr(p,'-'));p++) {
        ...
        trace(4,"getcodepri using opt: %s %s\n",optstr,str);
...
static int residual_test(rtk_t *rtk, const int *vflg, const double *v,
                         const double *Q, int *iv, const int m, int iter)
{
    int i,k,sat1,sat2,type,freq;
    char *stype;
    const double thres_adap=(rtk!=NULL)?rtk->opt.maxinno[0]:0.0;
    
  
    /* pre/post residuals test */
    for (i=k=0;i<m;i++) {
        if(rtk!=NULL&&vflg!=NULL&&iter>=rtk->opt.iterej){
            sat1=(vflg[i]>>16)& 0xFF;
            sat2=(vflg[i]>>8) & 0xFF;
            type=(vflg[i]>>4) & 0xF;
            freq=(vflg[i])    & 0xF;
            stype=(type==0)?"L":((type==1)?"P":((type==2)?"D":((type==3)?"D":"C")));
            const double thres_classic=(rtk!=NULL)?((type==0)?rtk->opt.maxinno[0]:((type==1)?rtk->opt.maxinno[1]:rtk->opt.maxinno[2])):0.0;
            /* validation for L1/L2 residuals */
            if (rtk->opt.adaprej&&(!Q[i+i*m]||thres_adap==0||(v[i]*v[i]<Q[i+i*m]*thres_adap*thres_adap))) {
                trace(2,"meas ok adap (sat=%2d-%2d %s%d v=%6.3f sig2=%6.3f outc=%2d iter=%d)\n",
                    sat1,sat2,stype,freq+1,v[i],Q[i+i*m],rtk->ssat[sat2-1].outc[freq],iter);
                iv[k++]=i;
            }
            else if(!rtk->opt.adaprej&&(fabs(v[i])<thres_classic)){
                trace(2,"meas ok classic (sat=%2d-%2d %s%d v=%6.3f sig2=%6.3f outc=%2d iter=%d)\n",
                    sat1,sat2,stype,freq+1,v[i],Q[i+i*m],rtk->ssat[sat2-1].outc[freq],iter);
                iv[k++]=i;
            } else {    
                /* only set sat as invalid if meas is phase or doppler*/
                char s1[8],s2[8];
                satno2id(sat1,s1);satno2id(sat2,s2);
                trace(2,"meas outlier detected %s (sat=%s-%s %s%d v=%6.3f sig2=%6.3f outc=%2d iter=%d)\n",rtk->opt.adaprej?"adaptive":"classic",
                    s1,s2,stype,freq+1,v[i],Q[i+i*m],rtk->ssat[sat2-1].outc[freq],iter);
                if (type==2||type==3){
...
*-----------------------------------------------------------------------------*/
static int filter_(rtk_t* rtk, const double *x, const double *P, const double *H,
                   const double *v, const double *R, int n, int m,
                   double *xp, double *Pp, double *Qp, const int *vflg, int iter)
{
    double *F=mat(n,m),*Q=mat(m,m),*K=mat(n,m),*I=eye(n),*vd=NULL;
    if(Qp!=NULL) {vd=mat(n,1);}
    int info=0;
    ...
    double *v_,*Q_,*F_,*H_;
    int i,j,k,*iv=imat(m,1);

     /* prefit/postfit residuals test */
    if (!(k=residual_test(rtk,vflg,v,Q,iv,m,iter))) {
        trace(2,"all measurements are rejected\n");
        matcpy(Pp,P,n,n);
        free(F); free(Q); free(K); free(I); free(iv);
        if(vd!=NULL) {free(vd);}
        return info;
    }

    /* construct system for accepted measurements */
    ...
    trace(2,"meas rejection, tot=%d acc=%d\n",m,k);

    /* solve system */
    if (!(info=matinv(Q_,k))) {
        matmul("NN",n,k,k,F_,Q_,K);   /* K=P*H*Q^-1 */
        matmulp("NN",n,1,k,K,v_,xp);  /* xp=x+K*v */
        matmulm("NT",n,n,k,K,H_,I);  /* Pp=(I-K*H')*P */
        matmul("NN",n,n,n,I,P,Pp);
        
        if(vd!=NULL&&rtk!=NULL&&rtk->opt.afgainion>0.0){
            trace(2,"calcualting adaptive gain, n=%2d m=%2d\n",n,k);
            matmul("NN",n,1,k,K,v_,vd);   /* vd=K*v */
            matmul("NT",n,n,1,vd,vd,Qp);   /* Q=vd(vd)' */
        }
        
    }

    trace(2,"system solved, info=%d\n",info);

    free(F); free(Q); free(K); free(I); 
    if(vd!=NULL) {free(vd);}
    free(v_); free(Q_); free(F_); free(H_); free(iv);
    trace(2,"filter memory freed\n");

    return info;
}
extern int filter(rtk_t *rtk, double *x, double *P, double *Q, const double *H, const double *v,
                  const double *R, int n, int m, const int *vflg, int iter)
{
    double *x_,*xp_,*P_,*Pp_,*Qp_=NULL,*H_;
    int i,j,k,info,*ix;
    ...
    info=filter_(rtk,x_,P_,H_,v,R,k,m,xp_,Pp_,Qp_,vflg,iter);
    /* copy values from compressed arrays back to full arrays */
    for (i=0;i<k;i++) {
        x[ix[i]]=xp_[i];
        for (j=0;j<k;j++) P[ix[i]+ix[j]*n]=Pp_[i+j*k];
        if(Q!=NULL) for (j=0;j<k;j++) Q[ix[i]+ix[j]*n]=Qp_[i+j*k];
    }
    free(ix); free(x_); free(xp_); free(P_); free(Pp_); free(H_);
    if(Qp_!=NULL){free(Qp_);}
    return info;
}
/* smoother --------------------------------------------------------------------
...
*          double *r        O   range distance [0] and range rate [1]
* return : 0: ok , 1: error
* notes  : distance includes sagnac effect correction
*-----------------------------------------------------------------------------*/
extern int geodist(const double *rs, const double *rr, double *e, double *r)
{
    double n,ev[3];
    int i;
    
    if (norm(rs,3)<RE_WGS84) return -1;
    /* Line-of-sight vector in ECEF */
    for (i=0;i<3;i++) e[i]=rs[i]-rr[i];
    n=norm(e,3);
    for (i=0;i<3;i++) e[i]/=n;
    /* range with earth rotation correction (sagnac) */
    r[0]=n+OMGE*(rs[0]*rr[1]-rs[1]*rr[0])/CLIGHT;
    /* satellite velocity relative to receiver in ECEF */
    for (i=0;i<3;i++) ev[i]=rs[i+3]-rr[i+3];
    /* range rate with earth rotation correction (sagnac) */
    r[1]=dot3(ev,e)+OMGE*(rs[4]*rr[0]+rs[1]*rr[3]-rs[3]*rr[1]-rs[0]*rr[4])/CLIGHT;
